...
app_server <- function( input, output, session ) {
  ...
  cities_in_current_country <- eventReactive(eventExpr = input$current_country_name,
                                             valueExpr = {
  ...
  current_streets_df_r <- reactive({
    ...
    if (is.null(current_streets_sf_r())) return(NULL)
  ...
  output$current_city_sn_dt <- DT::renderDT(expr = {
    if (is.null(current_streets_df_r())) return(NULL)
    DT::datatable(data = current_streets_df_r(),
                  filter = "top",
                  options = list(dom = "tp", 
                                 pageLength = 5,
                                 lengthMenu = c(3,5,
  ...
  output$current_city_map_leaflet <- leaflet::renderLeaflet({
    ...
    leaflet::leaflet(data = current_streets_sf_r()) %>%
      ...
      leaflet::addPolylines(color = "#ff5454",
                            weight = 3)
  })
