...
#'
#' @param country A country, either a two letter country code, or country full
#'   name. See `sn_country_codes` for valid values.
#' @param collapse_lists Defaults to FALSE. If TRUE, collapses multiple values
#'   in a single string, with each value separated by ";". Useful for exports in
#'   tabular format.
  ...
  all_df <- purrr::map_dfr(
    ...
    .f = function(x) {
      ...
      search_df <- sn_search_named_after(
        ...
        dplyr::mutate(
          named_after_label = tw_get_label(
            id = named_after_id,
            language = language,
            cache = cache,
            overwrite_cache = overwrite_cache,
          ...
          named_after_description = tw_get_description(
            id = named_after_id,
            language = language,
            cache = cache,
            overwrite_cache = overwrite_cache,
