...
sn_get_city_combo <- function(gisco_id,
  ...
  manually_checked_df <- sn_get_street_name_named_after_id(gisco_id = gisco_id,
                                                        country = country,
                                                        language = language,
  ...
  all_wikidata_df <- tw_get(id = c(automatically_checked_df$named_after_id),
                            language = language,
                            cache = TRUE,
  ...
  gender_df <- all_wikidata_df %>% 
    ...
    dplyr::mutate(gender = dplyr::case_when(value == "Q6581097" ~ "male",
                                            value == "Q6581072" ~ "female", 
                                            value %>% stringr::str_starts(string = , pattern = "Q") ~ "other")
    )  %>% 
    dplyr::group_by(id) %>% 
    dplyr::summarise(gender = dplyr::case_when("other" %in% gender ~ "other",
                                               dplyr::row_number()>1 ~ "other",
                                               TRUE ~ gender), .groups = "drop") %>% 
    dplyr::distinct() %>% 
