...
sn_create_map <- function(streets_sf = NULL,
  ...
  streets_data_df <- readr::read_csv(file = "/home/g/Downloads/RO_143450-municipiul sibiu (sibiu)-1668177211.csv")
  
  streets_sf <- latlon2map::ll_osm_get_lau_streets(gisco_id = "RO_143450",
                                                   unnamed_streets = unnamed_streets) %>% 
    dplyr::rename(street_name = name) %>% 
    dplyr::anti_join(y = streets_data_df %>% 
  ...
  streets_pre_sf <- streets_sf %>% 
    dplyr::left_join(y = streets_data_df %>% 
                       dplyr::filter(ignore==0) %>% 
                       dplyr::distinct(),
                     by = "street_name") %>% 
    dplyr::filter(is.na(street_name)==FALSE) %>% 
    dplyr::select(street_name,
                  named_after_id,
                  person,
                  gender)
  ...
  if (scope == "base") {
    ...
    factpal <- leaflet::colorFactor(palette = topo.colors(5),
    ...
    pre_yob_sf <- streets_pre_sf %>% 
      dplyr::mutate(year_of_birth = tw_get_p1(id = named_after_id,
                                              p = "P569") %>% 
